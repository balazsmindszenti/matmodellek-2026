...
class KMeansClustering:
    ...
    def prepare(self):
        ...
        # zip elementként párbaállít 2 adatstruktúrát -> tuple-ök
        # tuple-ökből dictionary -> dict
    ...
    def calculate_nearest_centroid_indexes(distances: np.ndarray) -> np.ndarray:
        # argmin megmondja, hogy hol van a minimum, nem a minimum értékét!
    ...
    def group_by_index(self, indexes: np.ndarray) -> dict:
        clusters = dict()
        for cl in range(0, self.k):
            clusters[cl] = self.data[indexes == cl]  # indexes elemeinek száma ugyanannyi, mint a data-nak
        return clusters
